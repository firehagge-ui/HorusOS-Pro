...
do Horus OS — como o Codex lê o contexto, aprende com correções, mantém
...
em `.Codex/skills/verificar/SKILL.md`. Se o comando não rodar (rede, node), o item
...
chat volta como erro no próximo site: o Codex não guarda nada entre conversas,
...
> `.Codex/skills/impeccable/SKILL.md`). O texto que dizia 3.5.0 era documentação
> vencida. **Para gerar versões visualmente divergentes** (não só reskin), o 4.0
> tem o torneio de conceitos: rodar `node .Codex/skills/impeccable/scripts/concept-seed.mjs
...
> primeiras tentativas do site do Grão da Serra. Ver a 20ª rodada no AGENTS.md dele.
...
|---|---|---|
| Skill (23 comandos) | `.Codex/skills/impeccable/` | ❌ dependência, 2 MB de terceiro |
| 4 agentes | `.Codex/agents/impeccable-*.md` | ✅ |
| 2 hooks | `.Codex/settings.json` | ✅ |
...
- ✅ Pro site da agência, o par vai em `site/`, junto do `AGENTS.md` e do
...
`.Codex/skills/<nome>/` é um guia de tokens de uma estética nomeada,
...
- `templates/` — modelos do Horus OS: perfis de `AGENTS.md`, catálogo de ferramentas
...
- `site/` — **site institucional da própria Hórus**, não de cliente. Por isso mora
  na raiz e não em `clientes/`. Tem `AGENTS.md` e `PLANO.md` próprios: ler os dois
