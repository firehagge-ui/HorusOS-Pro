...
- **Regras da casa:** `AGENTS.md` da raiz, os `AGENTS.md` de cada cliente, o `site/`
...
- Skills em `.Codex/skills/` — quais existem hoje. Bate com o que o `AGENTS.md` e o
  `README.md` descrevem?
...
- **`AGENTS.md` (raiz e cliente):** a estrutura de pastas descrita existe? As regras de
...
Varrer os `AGENTS.md` de cliente e os logs de rodada procurando regra generalizável, e
propor o destino:
...
- Regra de **como interpretar o pedido do Marcelo** ou de processo → `AGENTS.md` da raiz
...
nasceu no Grão da Serra e vale para todo cliente. Sobe para `preferencias.md` ou `AGENTS.md`.
...
CONTRADIÇÃO
2. design/README.md diz "X" e AGENTS.md diz "Y" sobre [tema]. O certo é Y porque [evidência]

FATO VENCIDO
3. estrategia.md fala em "apresentar em [data passada]" — já passou; marcar como histórico

PROMOÇÃO
4. grao-da-serra/AGENTS.md tem uma regra que vale pra todo cliente: [regra]. Subir pra [destino]
